...
It also carries the maintainer flows: onboard a site, maintain the template
catalog, sync the catalog to Webflow Agent Instructions, and resume or clean up
an interrupted run.

## Install

This repository is a plugin marketplace root:

- `.claude-plugin/marketplace.json` for Claude Code
- `.agents/plugins/marketplace.json` for Codex
- one plugin, `plugins/design-automations`, containing the skill

Claude Code and Codex install from this repository. Claude.ai does not: zip the
skill folder and upload it (or have a Team / Enterprise owner provision it). After
that, the catalog lives in Webflow, not git.
...
Invoke it by name: `/design-automations:webflow-template`. Plugin skills are
namespaced. The unqualified form (skill name only) works only for a copy
placed in `.claude/skills`.

The plugin ships `.mcp.json` pointing at `https://mcp.webflow.com/mcp`. Claude
Code prompts for the Webflow OAuth flow on first use.
...
Invoke with `$webflow-template`. `agents/openai.yaml` sets
`allow_implicit_invocation: false`, so Codex will not pick the skill up from
unrelated conversation.

### Claude.ai

Claude.ai never clones this repository. Zip the skill folder (not the
marketplace root) and upload it, or have a Team / Enterprise owner provision it.
After that, nobody needs GitHub: the catalog lives in Webflow Agent
Instructions.
...
Claude.ai rejects a zip whose folder name does not match the skill name. Send
the zip. Do not send the GitHub URL; a private repository will not help.
...
4. Connect the first-party Webflow connector: in a chat, **+** →
   **Connectors** → **Webflow**, complete OAuth, pick the site. The
   `agent_instructions:read` and `agent_instructions:write` scopes are what let
   the catalog live in Webflow; a workspace admin grants those.
...
written fallback the skill follows by hand when it is not.

End to end on Claude.ai, with no repository and no git:

1. **Onboard once.** Ask for onboarding by name. It probes the instruction
   store, measures the site read-only, proposes families, asks you to confirm
   them, and writes the catalog, filled conventions, and index into Webflow
   Agent Instructions. It asks before each write: that confirmation is the only
   review this path has.
2. **Keep the bundle.** Onboarding hands you the same content as one JSON file
   (or one HTML page). If a later build cannot read the store, attach the bundle
   and it will use it.
3. **Build.** Later conversations read the catalog, conventions, and rulebook
   from Webflow, and write the brief, run record, and candidates back as
   drafts.
4. **If the store is not readable**, the skill names the two missing scopes
   (`agent_instructions:read` and `agent_instructions:write`) and falls back to
   the bundle. It does not retry a 403 and it does not fail the run. With
   neither a store nor a bundle, a build stops at template selection and says
   so.
...
a site once, read-only, and generates:

- `references/catalog/<family>.md`, one entry per template family
- `references/webflow-conventions.md`, which ships as a template of
...
The measuring is identical in all three. Every path also hands over the same
content as a **download bundle** (one JSON file, or one HTML page; chat
accepts HTML, JSON, and plain text but not zip). A build accepts that bundle when
it cannot read the store.
...
bundle as the only history. `flows/maintain.md` describes promoting and
pruning through conversation when there is no repository.

Facts the flows depend on (request pacing, whether loose-section content is
reachable, whether branching is available, the breakpoint list, whether
component names are unique, whether the connector token can write page schema)
are not constants. Each is a MEASURE row in the conventions template. A row that
has not been measured stays UNMEASURED and is treated as unknown, not guessed.

A worked example for a fictional site (one catalog entry, a filled conventions
file, a brief, a rendered outline, and a dry-run manifest) lives under
...
`references/rules.md` rule 19; the same table is in `SKILL.md`.
...
(19 rules). It is what gets installed into the site's Agent Instructions so
...
- The six Python scripts do what their specs say. 118 unit tests cover brief
  ...
  path the docs point at exists, that the SKILL.md and CHANGELOG versions
  agree, and that no machine-specific path is committed.
- The worked example passes end to end: the brief validates with
  ...
  render of the brief, the catalog entry passes `catalog_lint.py`, and the
  guard fixtures produce exactly the pass and fail verdicts the manifest
  records.
- The plugin and marketplace manifests parse and carry the keys Claude Code
  and Codex need (`checks/repo-check.sh`, also run in CI), and `claude plugin
...
match, family rules, CTA, SEO, guard, and tracking), filled from live reads, not
from intent. Tracking gaps (missing UTMs, missing custom attributes, a missing
analytics shell the family already names) are Designer handoff work. The skill
does not invent click events or wire a new tracking scheme.

**Not verified, and stated as such wherever it matters:**

- **No live Webflow run is included.** The example is fictional. Nothing here
  is evidence that a build succeeds against any particular site; the first
  real evidence is your own onboarding run.
- **Branch-mode writes.** Onboarding can establish that element reads accept a
  branch page id. Whether writes on a branch page and `create_branch` without the
  Designer work is unknown until someone runs a branch-mode build.
...
- **Designer URL forms.** Only `designer_tool > switch_page` is known to
  navigate the connected Designer tab. Any `...design.webflow.com?pageId=`
  URL is a guess until someone checks it on the site in question.
...
**Measured outside this repository:** an anonymous request to a Webflow
branch staging URL redirects to the Webflow login and returns HTTP 403, so a
branch staging publish is gated behind a login rather than open to the
internet. That was checked against a live Webflow site; nothing in this tree
verifies it.
